...
def enrich_doctors_data(doctors_from_1c: list, target_branch: str = None) -> list:
    ...
    for doc in doctors_from_1c:
        ...
        if target_branch and found_info:
            ...
            # Если список филиалов НЕ пустой, и нужного филиала там НЕТ - пропускаем врача
...
def find_prodoctorov_url(doctor_name: str) -> str:
    """Ссылка на профиль врача на ПроДокторов или пустая строка."""
    target = _norm(doctor_name or "")
    if not target:
        return ""
    for key, info in DOCTORS_EXTRA_INFO.items():
        if _norm(key) in target:
            return info.get("prodoctorov_url", "") or ""
    return ""
