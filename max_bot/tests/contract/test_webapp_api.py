...
async def test_patient_normalized_for_onec(client, mocks):
    await client.post("/max/book", json=booking(), headers=auth(11))
...
async def test_bad_patient_field_422_without_onec(client, mocks, field, value, code):
    ...
    r = await client.post("/max/book", json=body, headers=auth(12))
...
async def test_onec_code_mapping(client, mocks, onec_code, expected):
    ...
    r = await client.post("/max/book", json=booking(), headers=auth(13))
...
async def test_onec_success_with_patient_fields(client, mocks, settings):
    ...
    r = await client.post("/max/book", json=booking(), headers=auth(14))
...
async def test_reschedule_onec_codes(client, mocks):
    await client.post("/max/book", json=booking(), headers=auth(15))
    for onec_code, expected in (
        ...
        r = await client.post(
            "/max/reschedule", json=booking(time_="11:00", old_id="appt-1"), headers=auth(15)
        )
    ...
    assert sent["platform"] == "max" and sent["patient"]["phone"] == "+79991234567"
