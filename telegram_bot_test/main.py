...
async def send_telegram_reminder(tg_id: str, text: str):
    ...
    try:
        async with httpx.AsyncClient(transport=transport, timeout=8.0) as http_client:
            ...
            if response.status_code == 200 and resp_data.get("ok"):
                print(f"📩 DEBUG: Сообщение доставлено в Telegram пациенту {tg_id}!")
                return
            else:
                print(f"❌ DEBUG: Telegram API ошибка: {resp_data}")
                return
    except Exception as e:
        print(f"⚠️ DEBUG: Сетевая ошибка Telegram: {repr(e)}")
        print(f"❌ DEBUG: Сообщение для {tg_id} не отправлено.")
...
async def lifespan(app: FastAPI):
    ...
    db.close()
    if restored_count > 0: print(f"✅ Восстановлено {restored_count} задач напоминаний")
...
@app.get("/doctors")
async def get_doctors(branch: str = None, date: str = None): return await client.get_doctors(branch, date)
...
async def book_appointment(req: BookingRequest, background_tasks: BackgroundTasks, send_notifications: bool = False, db: Session = Depends(get_db)):
    ...
    try:
        ...
        print(f"▶️ DEBUG: Попытка записи {req.patient.last_name} {req.patient.first_name} (telegram)...")
        ...
        print(f"❌ DEBUG: Ошибка при бронировании: {str(e)}")
...
def schedule_notifications(req, fio_short, formatted_date, branch_name, link_branch, link_map):
    try:
        ...
        if rem_24h > now:
            msg_24h = f"{fio_short}\n<b>{formatted_date}</b> вы записаны...\nПожалуйста, подтвердите ваши завтрашние приемы."
    ...
    except Exception as ex: print(f"❌ DEBUG: Ошибка при расчете времени: {ex}")
...
async def finish_visit(data: dict, db: Session = Depends(get_db)):
    ...
    if appt:
        ...
        msg_feedback = f"🌟 {fio}, надеемся, вам понравилось у нас!\n👉 <a href='{link}'>Оставить отзыв на Яндекс.Картах</a>"
        run_at = datetime.now() + timedelta(minutes=20)
        scheduler.add_job(send_telegram_reminder, 'date', run_date=run_at, args=[appt.tg_id, msg_feedback], id=f"feedback_{appt_id}", replace_existing=True)
        return {"status": "success"}
...
async def telegram_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    except Exception as e: print(f"❌ Ошибка при обработке вебхука: {e}")
    return {"status": "ok"}
