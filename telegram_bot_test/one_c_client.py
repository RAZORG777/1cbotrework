...
class OneCClient:
    ...
    async def get_schedule(self, doctor_id: str, date: str, branch: str = None):
    ...
    # Метод для отмены записи в 1С
    async def cancel_booking(self, appointment_id: str):
        payload = {"appointment_id": appointment_id} # Передаем ID записи
