...
async def test_finish_then_book_again(client, app, mocks):
    assert (await client.post("/book", json=booking(), headers=auth(5))).json()[
        "status"
    ] == "success"
    r = await client.post(
        "/api/v1/internal/finish-visit", json={"appointment_id": "appt-1"}, headers=SIG
    )
    ...
    r = await client.post("/book", json=booking(), headers=auth(5))
...
async def test_cancel_then_book_again(client, mocks):
    ...
    assert (await client.post("/cancel", headers=auth(5))).json()["status"] == "success"
    mocks["book"].respond(json={"status": "success", "appointment_id": "appt-9"})
    assert (await client.post("/book", json=booking(), headers=auth(5))).json()[
        "status"
    ] == "success"
...
async def test_past_visit_does_not_block(client, app, mocks):
    ...
    assert (await client.get("/my_appointment", headers=auth(5))).json()["has_appointment"] is False
    mocks["book"].respond(json={"status": "success", "appointment_id": "appt-9"})
    assert (await client.post("/book", json=booking(), headers=auth(5))).json()[
        "status"
    ] == "success"
