...
- **Decision**: Модуль HTTP-сервиса УМЦ (методы `BookPOST`, `CancelPOST`, `ReschedulePOST`,
  ...
  задачей T063. Отправка `cancel-visit` найдена в модуле регистра сведений
  «ПричиныОтменыЗаявок» (`ОтправитьСигналБотуОбОтменеНаСервере`), место `finish-visit` ищет
  владелец. Модуль объекта документа «Заявка» (`onec/Документ.Заявка.МодульОбъекта.bsl`)
  вызовов бота тоже не содержит. Значит, сигнал отправляется из расширения конфигурации или
  из общего модуля по подписке на событие.
