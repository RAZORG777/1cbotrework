...
- **Decision**: 1С передаёт заголовок `X-Bot-Secret: <ONEC_WEBHOOK_SECRET>` в
  ...
  Сигналы идут по loopback (`127.0.0.1:8001` и `127.0.0.1:8002`): 1С и боты на одном сервере.
  Сейчас `cancel-visit` отправляется только в Telegram-бот, это исправляется в T063
  (contracts/onec-signals.md).
...
- **Decision**: `pydantic-settings` в каждом боте, класс `Settings` с обязательными полями:
  ...
  `PD_RETENTION_DAYS=30`, `INIT_DATA_MAX_AGE_HOURS=24`, `ADMIN_IDS`, `DB_PATH`, `LOG_DIR`, `HOST=127.0.0.1`,
  `PORT`.
...
- **Rationale**: Сейчас TG ссылается на `yasno-vizhu.com/privacy-policy/`, а MAX — на PDF
  политики. Владелец выбрал единый документ: `PD_POLICY_URL=https://yasno-vizhu.com/docs/Согласие_на_обработку_персональных_данных.pdf`.
- **Сверка с документом**: срок хранения в согласии — «не более 30 дней с даты достижения цели
  обработки», это совпадает с `PD_RETENTION_DAYS=30`. Но в перечне данных нет **даты рождения**,
  а среди целей нет записи на приём и мессенджеров (Telegram, MAX). Бот собирает дату рождения,
  поэтому клинике нужно дополнить текст согласия (конституция › TODO(PD_CONSENT_SCOPE)).
  Код это не блокирует, но это условие выкладки в прод. Не юридическое заключение.
...
- **Адрес прослушивания**: сейчас Uvicorn слушает `0.0.0.0`, и служебные эндпоинты доступны
  из сети напрямую, минуя HTTPS. Добавляется настройка `HOST` (по умолчанию `127.0.0.1`).
  Если обратный прокси стоит на другом хосте, владелец явно указывает адрес.
...
## R16. Конфигурация 1С (распакована из `1Cv8.cf`, БИТ.УМЦ 2.0.49.101)

Конфигурация выгружена владельцем в `_1c_dump/1Cv8.cf` (в git не попадает) и распакована
локально, собственным распаковщиком на stdlib. Выводы:

- **HTTP-сервис** (`onec/http-service.bsl`) — источник истины для `docs/onec-contract.md`
  и контрактных тестов клиента 1С (T029, T034, T035).
- **Сигналы боту**: единственное обращение — модуль формы записи регистра «ПричиныОтменыЗаявок»,
  только `cancel-visit`, только в Telegram, только если в причине или комментарии есть «бот».
  `finish-visit` нет нигде; сигнал проверялся вручную через `test.py`, поэтому просьбы об отзыве
  пациентам не отправлялись. Новые точки срабатывания — contracts/onec-signals.md (T063, T064).
- **Документ «Прием»**: связь с заявкой — `ДокументОснование` (заполняется при создании приёма
  из заявки). При проведении `КалендарьПланирования.ИзменитьСостояниеЗаявкиПоПодчиненнымДокументам`
  переводит заявку в «Прием» или «Выполнена». Реквизит «Закрыт» — «первичный приём закрыт
  (случай обращения завершён)», для сигнала не подходит.
- **Причина проблемы с ЭМК (этап 1), подтверждена кодом:**
  1. `BookPOST` записывает клиента с `ОбменДанными.Загрузка = Истина`. В этом режиме
     `Справочник.Клиенты.ПриЗаписи` сразу выходит и не вызывает штатное
     `Справочники.Клиенты.СоздатьМедкартуКлиента` (автосоздание медкарты по учётной политике
     «АвтоСозданиеМедкарт», номер через `ОтраслевыеДополнения.ФормированиеНомера`, установка
     `ОсновнаяМедицинскаяКарта`). `ПередЗаписью` при этом пропускает нормализацию ФИО
     (`КонтрольКорректностиФИО`), `Автор` и `ДатаСоздания`.
  2. Самодельный блок создания медкарты в `BookPOST` ищет карту по полю `Пациент`, а в справочнике
     «МедицинскиеКарты» поле называется `Клиент`. Запрос падает с ошибкой, её глушит
     `Попытка … Исключение КонецПопытки`, и **медкарта не создаётся никогда**.
  3. Итог: у клиента из бота нет медкарты и основной медкарты, поэтому ЭМК оформить нельзя,
     а номер карты не появляется.
  - **Решение для этапа 1**: записывать клиента без режима `Загрузка` (штатные обработчики
    создадут медкарту с номером); для найденного существующего клиента без
    `ОсновнаяМедицинскаяКарта` вызывать `Справочники.Клиенты.СоздатьМедкартуКлиента(Клиент, ,
    Ложь)`; самодельный блок нумерации удалить; ошибки не глушить, а возвращать боту кодом.
- **Прочие наблюдения для этапа 1**: поиск клиента — только точное ФИО + дата рождения
  (при нераспознанной дате — `01.01.0001`, риск привязки к чужому клиенту); перенос всегда
  30 мин и примечание «Перенос из Telegram» даже для MAX; ошибки возвращают `ОписаниеОшибки()`.
